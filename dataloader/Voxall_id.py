...
class myImageloader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_idx = random.randint(0, len(id_img_list)-1)
        ...
        # select another identity's frame
        dir_idx = random.randint(0, self.__len__-1)
        identity_dir2 = self.alldatalist[dir_idx] + '/'
        id_img_list2 = [identity_dir2 + '/' + img.name for img in os.scandir(identity_dir2) if is_image_file(img.name)]   
        img_idx2 = random.randint(0, len(id_img_list2)-1)
        img2 = self.loader(id_img_list2[img_idx2])
        while img2 is None:
            img_idx2 = random.randint(0, len(id_img_list2)-1)
            img2 = self.loader(id_img_list[img_idx2])
        img2 = processed1(img2)
